<?php
    ...
    // datas festivas e seu respectivo texto
    $datas = array(
        '25/12' => 'um Feliz Natal!',
        calcDataPascoa() => 'uma Feliz Páscoa!',
        '01/01' => 'um Feliz Ano Novo!',
    );

    // verifica se hoje é uma data festiva
    // se é, pega o texto a ser colocado no sms
    // se não, para
    if (!in_array(date('d/m'), array_keys($datas)))
        die;
    else
        $subst = $datas[date('d/m')];
    ...
    foreach ($arrClinicas as $arrClinica) {
        ...
        if (!$configuracoes['indDatasFestivas'])
            continue;
