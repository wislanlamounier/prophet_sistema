...
<form action="<?php echo BASE_URL; ?>/sms/historico/" method="post">
<?php }else{ ?>
<form action="<?php echo BASE_URL; ?>/sms/baixarHistoricoFim/" method="post" target="_blank">
<?php } ?>
    <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3 form-group">
            <label for="tipo" class="control-label">Mostrar:</label> <br>
            <input <?php echo $tipo2 == 'todos' || is_null($tipo2) ? 'checked' : ''; ?> type="radio" name="tipo" class="tipo" value="todos"> Todos
            &nbsp;&nbsp;&nbsp;
            <input <?php echo $tipo2 == 'aviso_consulta' ? 'checked' : ''; ?> type="radio" name="tipo" class="tipo" value="aviso_consulta"> Aviso de consulta
            <br>
