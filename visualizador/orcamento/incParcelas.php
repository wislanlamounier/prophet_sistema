<div class="col-md-12 form-group">
    ...
    <input type="number" name="diaVencimento" class="form-control" disabled
           value="<?php echo $dtoOrcamentoFormaPagamento->getNumDiaVencimento(); ?>">
</div>
<div class="row">
                                ...
                                if (!$dtoOrcamentoParcela->getIndPaga()) {
                                    if ($dtoOrcamento->getIndAprovado()) {
                                        ...
                                        if ($dtoOrcamentoFormaPagamento->getIndVia() == 'boleto') {
                                            ?>
                                            <a target="_blank"
                                               href="<?php echo BASE_URL; ?>/boleto/gerar/<?php echo $dtoOrcamentoParcela->getValParcela() . '/' . 'orcamento_parcela/' . $dtoOrcamento->getCdnPaciente() . '/' . $dtoOrcamento->getCdnOrcamento() . '/' . $dtoOrcamentoParcela->getNumParcela(); ?>">
                                                <button type="button" class="btn btn-primary">
                                                    Gerar boleto
                                                </button>
                                            </a>
                                            <?php
                                        } elseif($dtoOrcamentoFormaPagamento->getIndVia() == 'nota'){
                                                ?>
                                                <a target="_blank"
                                                   href="<?php echo BASE_URL; ?>/orcamento/notaPromissoria/<?php echo $dtoOrcamento->getCdnOrcamento() . '/' . $dtoOrcamentoParcela->getNumParcela(); ?>">
                                                    <button type="button" class="btn btn-primary">
                                                        Gerar nota promissória
                                                    </button>
                                                </a>
                                                <?php
                                        } elseif($dtoOrcamentoFormaPagamento->getIndVia() == 'autorizacaoDesc'){
                                            ?>
                                            <a target="_blank"
                                               href="<?php echo BASE_URL; ?>/orcamento/autorizacaoDesconto/<?php echo $dtoOrcamento->getCdnOrcamento() . '/' . $dtoOrcamentoParcela->getNumParcela(); ?>">
                                                <button type="button" class="btn btn-primary">
